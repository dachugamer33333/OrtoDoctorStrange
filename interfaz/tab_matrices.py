...
def _fmt_matriz(M):
    lines = []
    for row in M:
        parts = "  " + " ".join(f"{x:>8.4f}" for x in row)
        lines.append(parts)
    return "[\n" + "\n".join(lines) + "\n]"
...
def crear_tab_matrices(tabview):
    ...
    def diagnosticar():
        ...
        try:
            ...
            if not isinstance(matrices, list) or any(not isinstance(M, (list, tuple)) for M in matrices):
                raise ValueError
        ...
        num = arr.shape[0]
        rango = np.linalg.matrix_rank(arr.reshape(num, m * n_val))
        li = rango == num
        gen = rango == dim_espacio
        base = li and gen
        ort = mat.proseso_ortogonal(arr)
        orton = mat.proseso_ortonormal(arr, ort)

        # Mostrar matrices dadas
